...
do
	...
	local function iter(t, prestate) -- Azeroth / Zone / Minimap / Inside Dungeon settings
      ...
      if t.uiMapId == 1670 or t.uiMapId == 125 then -- Oribos scale
          scale = db.zoneScale
          alpha = db.zoneAlpha
          
          if (t.uiMapId == 1670 or t.uiMapId == 125) and t.minimapUpdate then -- Oribos Minimap scale
            scale = db.minimapScale
            alpha = db.minimapAlpha
          end
      end      
...
function Addon:PLAYER_LOGIN()
  ...
  self.db = LibStub("AceDB-3.0"):New("HandyNotes_MapNotesCataDB", ns.defaults, true)
  db = self.db.profile
